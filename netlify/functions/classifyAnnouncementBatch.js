...
export async function handler(event) {
  ...
  try {
    const { announcements } = JSON.parse(event.body)
...
제목: ${ann.title}
기관: ${ann.organization || '-'}
요약: ${(ann.summary || '').slice(0, 100)}
태그: ${(ann.tags || []).slice(0, 5).join(', ') || '-'}`
    }).join('\n\n')

    const systemPrompt = `정부지원사업 공고를 산업 분야로 분류하는 전문가입니다. 여러 공고를 한 번에 분류합니다. JSON 배열로만 응답하세요.`

    const userPrompt = `다음 ${batch.length}개 공고를 분류하세요.
